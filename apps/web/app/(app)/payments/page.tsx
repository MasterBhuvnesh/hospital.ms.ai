    ...
    <>
      ...
      {invoices.length === 0 ? (
        ...
        <Card className="gap-0 bg-muted/50 p-1 ring-0 shadow-sm">
          ...
          <div className="rounded-xl bg-card font-[350] [&_details+details]:border-t [&_details+details]:border-border-subtle">
            {invoices.map((inv) => (
              <details key={inv.id} open={inv.status === "UNPAID"} className="group">
                <summary className="flex cursor-pointer list-none flex-wrap items-center gap-x-3 gap-y-2 px-4 py-4 transition-colors duration-120 ease-out hover:bg-muted/50 sm:px-5 [&::-webkit-details-marker]:hidden">
                  <StatusBadge status={inv.status} />
                  <span className="font-mono text-sm font-[450] text-foreground">{inv.invoiceNo}</span>
                  <span className="min-w-0 grow truncate text-sm text-muted-foreground">{fmtDateTime(inv.createdAt)}</span>
                  <span className="whitespace-nowrap font-mono font-[450] text-foreground">{money(inv.total, inv.currency)}</span>
                  <ChevronDown
                    aria-hidden
                ...
                <div className="px-4 pb-4 sm:px-5 sm:pb-5">
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-3">
                    {inv.status === "UNPAID" && (
                      <Button variant="cta" size="sm" disabled={paying === inv.id} onClick={() => pay(inv)}>
      ...
      {payments.length > 0 && (
        <section className="mt-8">
          ...
          <Card className="gap-0 bg-muted/50 p-1 ring-0 shadow-sm">
            <div className="flex items-center justify-between px-3 py-2">
              <PanelTitle title="Payment history" />
              <MoreButton />
            </div>
            <div className="overflow-hidden rounded-xl bg-card py-2">
              <Table className="font-[350]">
                <TableHeader>
                  <TableRow className="hover:bg-transparent">
                ...
                <TableBody>
                  {payments.map((p) => (
                    <TableRow key={p.id}>
                      <TableCell className="whitespace-nowrap font-mono text-body-small">{fmtDateTime(p.capturedAt)}</TableCell>
