    ...
    <div className="dash">
      ...
      <main className="main">
        ...
        <div className="content fade-in">
          ...
          {tab === "messages" && <MessagesTab store={store} />}
...
// ─── MESSAGES TAB ───────────────────────────────────────────────────────────
function MessagesTab({ store }: any) {
  const user = store.user || {};
  const userId = user.id || '';
  ...
  const getContactName = (m: any, otherId: string) => {
  ...
  const loadConversations = useCallback(() => {
    ...
    fetch('/api/messages?userId=' + userId)
      ...
      .then(data => {
        ...
        const list = Object.values(contactMap).sort((a: any, b: any) => new Date(b.lastTime).getTime() - new Date(a.lastTime).getTime());
        setContacts(list);
        setLoading(false);
      })
      .catch(() => setLoading(false));
  ...
  const openConversation = (contact: any) => {
    ...
    fetch(`/api/messages?userId=${userId}&otherUserId=${contact.id}`)
      ...
      .then(data => {
        if (Array.isArray(data)) {
          setMessages(data);
          data.filter((m: any) => m.receiverId === userId && !m.read).forEach((m: any) => {
  ...
  const startConversation = (doctor: any) => {
    const existing = contacts.find(c => c.id === doctor.userId);
    if (existing) {
      openConversation(existing);
    } else {
      setSelectedContact({ id: doctor.userId, name: doctor.name });
      setMessages([]);
      setShowNewChat(false);
    }
  };
  ...
  if (loading) return <div style={{ padding: 40, textAlign: 'center', color: 'var(--muted)' }}>Loading messages…</div>;

  return (
    <div>
      <div className="page-header">
        <div className="page-tag">Communication</div>
        <h1 className="page-title">Messages</h1>
        <p className="page-sub">Chat with your doctors and healthcare team.</p>
      </div>
      ...
      <div className="card" style={{ padding: 0, display: 'flex', height: 520, overflow: 'hidden' }}>
        ...
        <div style={{ width: 280, borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button onClick={openNewChat} title="New conversation" style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--blue)', color: 'white', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Icon name="edit" style={{ fontSize: 14 }} />
            </button>
          </div>
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {contacts.length === 0 && !showNewChat && (
              <div style={{ padding: 24, textAlign: 'center', color: 'var(--muted)', fontSize: 13 }}>No conversations yet.<br /><button onClick={openNewChat} style={{ color: 'var(--blue-mid)', background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, marginTop: 8 }}>Start a new conversation</button></div>
            )}
            {contacts.map(c => (
              <button
                key={c.id}
                ...
                style={{
                  ...
                  background: selectedContact?.id === c.id ? 'var(--blue-light)' : 'transparent',
                  border: 'none', borderBottom: '1px solid var(--border)', cursor: 'pointer', textAlign: 'left'
                }}
              >
                <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--navy)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
                  {getInitials(c.name)}
                </div>
                <div style={{ flex: 1, overflow: 'hidden' }}>
                  ...
                  <div style={{ fontSize: 11.5, color: 'var(--muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.lastMsg}</div>
                </div>
                {c.unread > 0 && (
                  <span style={{ minWidth: 18, height: 18, borderRadius: 9, background: 'var(--blue)', color: 'white', fontSize: 9, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{c.unread}</span>
                )}
              </button>
            ))}
          </div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          {showNewChat ? (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
              <div style={{ padding: '12px 18px', borderBottom: '1px solid var(--border)', fontWeight: 600, fontSize: 14, color: 'var(--navy)' }}>New Conversation</div>
              <div style={{ padding: 16, flex: 1, overflowY: 'auto' }}>
                <p style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 12 }}>Select a doctor to message:</p>
                {doctorsList.length === 0 && <p style={{ fontSize: 13, color: 'var(--muted)' }}>Loading doctors…</p>}
                {doctorsList.map(doc => (
                  <button
                    key={doc.id}
                    onClick={() => startConversation(doc)}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', cursor: 'pointer', marginBottom: 8, textAlign: 'left' }}
                  >
                    <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--navy)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700 }}>
                      {doc.img || doc.name?.slice(4, 6) || 'Dr'}
                    </div>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--navy)' }}>{doc.name}</div>
                      <div style={{ fontSize: 11.5, color: 'var(--muted)' }}>{doc.specialty}</div>
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted)', fontSize: 14, flexDirection: 'column', gap: 8 }}>
              <Icon name="chat_bubble_outline" style={{ fontSize: 40, opacity: 0.3 }} />
              <span>Select a conversation or start a new one</span>
            </div>
          ) : (
            <>
              <div style={{ padding: '12px 18px', borderBottom: '1px solid var(--border)', fontWeight: 600, fontSize: 14, color: 'var(--navy)' }}>
                {selectedContact.name}
              </div>
              <div style={{ flex: 1, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
                {messages.length === 0 && <div style={{ textAlign: 'center', color: 'var(--muted)', fontSize: 13, marginTop: 40 }}>No messages yet. Say hello!</div>}
                {messages.map((m: any, i: number) => (
                  <div key={m.id || i} style={{
                    alignSelf: m.senderId === userId ? 'flex-end' : 'flex-start',
                    maxWidth: '75%', padding: '9px 13px', borderRadius: 13,
                    background: m.senderId === userId ? 'var(--navy)' : 'var(--bg)',
                    color: m.senderId === userId ? 'white' : 'var(--text)',
              ...
              <div style={{ padding: '10px 14px', borderTop: '1px solid var(--border)', display: 'flex', gap: 8 }}>
                <input
                  ...
                  style={{ flex: 1, border: '1.5px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '9px 13px', fontSize: 13.5, outline: 'none', background: 'var(--bg)' }}
                />
                <button onClick={sendMessage} style={{ background: 'var(--navy)', color: 'white', width: 38, height: 38, borderRadius: 'var(--radius-sm)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', cursor: 'pointer' }}>
