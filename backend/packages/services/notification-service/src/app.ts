...
fastify.post('/notifications', { schema: { body: { type: 'object', properties: { type: { type: 'string' }, recipient: { type: 'string' }, message: { type: 'string' } } } }}, async (request, reply) => {
  await reply.send({ message: 'Send notification - ' + JSON.stringify(request.body) });
});
