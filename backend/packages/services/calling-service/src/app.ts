...
fastify.post('/calls', { schema: { body: { type: 'object', properties: { patientId: { type: 'string' }, doctorId: { type: 'string' } } } }}, async (request, reply) => {
  await reply.send({ message: 'Create call - ' + JSON.stringify(request.body) });
});

fastify.get('/health', async (request, reply) => {
  return { status: 'ok', service: 'calling-service' };
});
