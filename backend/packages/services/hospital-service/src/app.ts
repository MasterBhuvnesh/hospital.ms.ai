...
fastify.post('/hospitals', { schema: { body: { type: 'object', properties: { name: { type: 'string' }, location: { type: 'string' } } } }}, async (request, reply) => {
  await reply.send({ message: 'Create hospital - ' + JSON.stringify(request.body) });
});
