...
fastify.post('/results', { schema: { body: { type: 'object', properties: { patientId: { type: 'string' }, testName: { type: 'string' }, result: { type: 'string' } } } }}, async (request, reply) => {
  await reply.send({ message: 'Add lab result - ' + JSON.stringify(request.body) });
});

fastify.get('/health', async (request, reply) => {
  return { status: 'ok', service: 'lab-result-service' };
});
