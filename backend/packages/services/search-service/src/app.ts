...
fastify.post('/search', { schema: { body: { type: 'object', properties: { query: { type: 'string' } } } }}, async (request, reply) => {
  await reply.send({ message: 'Search results - ' + JSON.stringify(request.body) });
});
