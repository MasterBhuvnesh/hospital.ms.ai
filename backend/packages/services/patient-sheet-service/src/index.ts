...
/**
 * @fileoverview Patient Sheet Service
...
import patientSheetRoutes from './routes/patient-sheet.routes.js';
...
const logger = createLogger({
  serviceName: 'patient-sheet-service',
  level: 'info',
  enableConsole: true,
  enableFile: false,
});
...
app.use('/patient-sheets', patientSheetRoutes);
...
app.listen(PORT, () => {
  logger.info('Patient sheet service started', {
    port: PORT,
    environment: process.env.NODE_ENV || 'development',
  });
  logger.info(`Patient sheet service running on port ${PORT}`, {
