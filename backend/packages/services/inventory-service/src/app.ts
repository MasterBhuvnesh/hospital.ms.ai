...
fastify.post('/items', { schema: { body: { type: 'object', properties: { name: { type: 'string' }, quantity: { type: 'number' } } } }}, async (request, reply) => {
  await reply.send({ message: 'Add inventory item - ' + JSON.stringify(request.body) });
});
